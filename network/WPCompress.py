...
class WPCompression:
    ...
    def compress(self,source):
        ...
        for i in range(0,size):
            nrBits = (int.from_bytes(self.huffmanTable(source[i][0]), byteorder="big") - 1)
            val = int.from_bytes(self.huffmanTable(source[i][1]), byteorder="big")
        ...
        nrBits = (int.from_bytes(self.huffmanTable([256][0]), byteorder="big") - 1)
        val = int.from_bytes(self.huffmanTable([256][0]), byteorder="big")
    ...
    def createTree(self):
        ...
        while i < 257:
            ...
            nrBits = (int.from_bytes(self.huffmanTable([i][0]), byteorder="big") - 1)
            val = int.from_bytes(self.huffmanTable([i][0]), byteorder="big")
